...
void altaPersona(eDuenio lista[],int tam,int maximo)
{
    ...
    else
    {
        ...
        else
        {
            ...
            while(validarNumero(buffer)!=0)
            {
                printf("\nReingrese el nº de la tarjeta de credito: ");
                fflush(stdin);
                gets(buffer);

            }
            tarj=atoi(buffer);
            nuevaPersona.numTarjCred=tarj;
...
int buscarPatente(eEstacionamiento lista[], char patente[], int tam)
{
    int indice=-1;
    int i;
    for(i=0; i<tam; i++)
...
void altaEstacionamiento(eEstacionamiento lista[],int tam,int maximo)
{
    ...
    else
    {
        ...
        esta=buscarPatente(lista,nuevoauto.patente,tam);
        if(esta!=-1)
        {
...
void modificarDuenio(eDuenio pers[],int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        if(id==pers[i].idDuenio)
        {
            ...
            while(validarNumero(buffer)!=0)
            {
                printf("Reingrese el nuevo numero de tarjeta: ");
                fflush(stdin);
                gets(buffer);

            }
            tarj=atoi(buffer);
            pers[i].numTarjCred=tarj;
            flag=1;
            break;
        }
